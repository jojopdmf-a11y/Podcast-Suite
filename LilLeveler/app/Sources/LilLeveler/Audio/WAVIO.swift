...
enum WAVIO {
    ...
    static func load(url: URL) throws -> Buffer {
        ...
        return try data.withUnsafeBytes { (raw: UnsafeRawBufferPointer) -> Buffer in
            ...
            guard let pcm = pcmData else { throw LevelerError.loadFailed("No data chunk") }
            guard fmtAudioFormat == 1 || fmtAudioFormat == 3 else {
                throw LevelerError.loadFailed("Only PCM/float WAV supported")
            }
            ...
            if fmtAudioFormat == 3, fmtBits == 32 {
                ...
                throw LevelerError.loadFailed("Unsupported bit depth \(fmtBits)")
            }
