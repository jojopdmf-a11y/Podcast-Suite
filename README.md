...
- **Fixer Mixer** — **SAVE MIX** writes `FixerMixer.mix.json` next to the tracks. Drop that folder later and the mix comes back. **LOAD MIX…** opens a mix file. You can also drop any audio files (each file becomes a channel), or **NEW SESSION** then **RECORD** from your interface. **ALL TRACKS** stacks every waveform in one view; click a lane to zoom in on that strip. Shift-drag the waveform to mute a cough (silence, same length — not a cut).
